...
let%expect_test "pure literal elimination test" =
  let clauses = [[Pos "A"; Pos "B"]; [Pos "A"; Neg "C"]; [Pos "B"; Neg "C"]] in
  let result = clauses |> dp in
  print_endline (show_status result);
  [@expect {||}];
  [%expect {| Sat.Sat |}]
...
let%expect_test "unit propagation creates satisfiable result" =
  let clauses = [[Pos "A"]; [Neg "A"; Pos "B"]; [Neg "A"; Neg "B"; Pos "C"]] in
  let result = clauses |> dp in
  print_endline (show_status result);
  [@expect {||}];
  [%expect {| Sat.Sat |}]
...
let%expect_test "small unsatisfiable formula" =
  let clauses = [[Pos "A"; Pos "B"]; [Neg "A"; Pos "B"]; [Pos "A"; Neg "B"]; [Neg "A"; Neg "B"]] in
  let result = clauses |> dp in
  print_endline (show_status result);
  [@expect {||}];
  [%expect {| Sat.Unsat |}]  
...
let%expect_test "multiple unit clauses in sequence" =
  let clauses = [[Pos "A"]; [Pos "B"]; [Pos "C"]; [Neg "A"; Neg "B"; Neg "C"; Pos "D"]] in
  let result = clauses |> dp in
  print_endline (show_status result);
  [@expect {||}];
  [%expect {| Sat.Sat |}]
...
let%expect_test "resolution creates tautology" =
  let clauses = [[Pos "A"; Pos "B"]; [Neg "A"; Neg "B"]; [Pos "A"; Neg "B"]; [Neg "A"; Pos "B"]] in
  let result = clauses |> dp in
  print_endline (show_status result);
  [@expect {||}];
  [%expect {| Sat.Unsat |}]
...
let%expect_test "pure literal with multiple occurrences" =
  let clauses = [[Pos "A"; Pos "B"]; [Pos "A"; Pos "C"]; [Pos "A"; Pos "D"]; [Neg "B"; Neg "C"; Neg "D"]] in
  let result = clauses |> dp in
  print_endline (show_status result);
  [@expect {||}];
  [%expect {| Sat.Sat |}]
...
let%expect_test "mixed unit propagation and pure literal elimination" =
  let clauses = [[Pos "A"]; [Neg "A"; Pos "B"; Pos "C"]; [Pos "D"]; [Neg "D"; Pos "E"]] in
  let result = clauses |> dp in
  print_endline (show_status result);
  [@expect {||}];
  [%expect {| Sat.Sat |}]
...
let%expect_test "resolution with longer clauses" =
  ...
  let clauses = [[Pos "A"; Pos "B"; Pos "C"]; [Neg "A"; Pos "D"; Pos "E"]; [Neg "B"; Neg "C"; Neg "D"; Neg "E"]] in
  let result = clauses |> dp in
  print_endline (show_status result);
  [@expect {||}];
  [%expect {| Sat.Sat |}]
...
let%expect_test "duplicate literals in clauses" =
  ...
  let clauses = [[Pos "A"; Pos "A"; Pos "B"]; [Neg "A"; Neg "A"]; [Neg "B"; Pos "C"; Pos "C"]] in
  let result = clauses |> dp in
  print_endline (show_status result);
  [@expect {||}];
  [%expect {| Sat.Sat |}]
...
let%expect_test "three variable satisfiable" =
  ...
  let clauses = [[Pos "A"; Pos "B"; Pos "C"]; [Neg "A"; Pos "B"]; [Neg "B"; Pos "C"]; [Neg "C"]] in
  let result = clauses |> dp in
  print_endline (show_status result);
  [@expect {||}];
  [%expect {| Sat.Unsat |}]
...
let%expect_test "pigeonhole principle 2 pigeons 1 hole" =
  ...
  let clauses = [[Pos "P1"; Pos "P2"]; [Neg "P1"; Neg "P2"]; [Pos "P1"]; [Pos "P2"]] in
  let result = clauses |> dp in
  print_endline (show_status result);
  [@expect {||}];
  [%expect {| Sat.Unsat |}]
